    ...
    @if (auth()->user()->hasPermissionTo('Edit Landing'))
        <div class="container-xxl flex-grow-1 container-p-y">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h4 data-i18n="Landing section" style="margin: 0;"> header section</h4>
                @if (auth()->user()->hasPermissionTo('Edit Landing'))
                    <a class="btn btn-primary " style="margin: 0;" href={{ route('header-sections.create') }}
                        data-i18n="Add New Header">
                        Add New Header</a>
                @endif
            </div>
            <div class="row">
                    ...
                    <tbody>
                        @foreach ($headers as $header)
                            <tr>
                                ...
                                <td>
                                    <img src="{{ asset('images/HeaderSections/' . $header->image) }}" alt=""
                                        width="150px" height="150px">
                                </td>
                                @if (auth()->user()->hasPermissionTo('Edit Landing'))
                                    <td>
                                        <form
                                            action="{{ route('header-sections.destroy', ['header_section' => $header->id]) }}"
