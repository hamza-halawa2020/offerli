...
@section('title')
    <title data-i18n="SubCategories">SubCategories</title>
@endsection
    ...
    @if (auth()->user()->hasPermissionTo('View Categories'))
        <div class="container-xxl flex-grow-1 container-p-y">
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <h4 data-i18n="SubCategories" style="margin: 0;"> SubCategories</h4>
                @if (auth()->user()->hasPermissionTo('Add Category'))
                    <a class="btn btn-primary " style="margin: 0;" href={{ route('subcategories.create') }}
                        data-i18n="Add New SubCategory">
                        Add New SubCategory</a>
                @endif

            </div>
            <div class="row">
                <table id="SubCategories" class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th data-i18n="Logo">Logo</th>
                            <th data-i18n="SubCategory">SubCategory</th>
                            ...
                            {{-- @if (auth()->user()->hasPermissionTo('Add Category'))
                                <th>
                                    <a class="btn btn-primary " href={{ route('subcategories.create') }} data-i18n="add">
                                        Add</a>
                                </th>
                            @endif --}}
                    ...
                    <tbody>
                        @foreach ($subcategories as $subcategory)
                            <tr>
                                ...
                                <td><a
                                        href="{{ route('subcategories.edit', $subcategory->slug) }}">{{ ucfirst($subcategory->name) }}</a>
                                </td>
                                <td> <a href="{{ route('subcategories.show', $subcategory->slug) }}">{{ $subcategory->category->name }}
                                    </a></td>
                                @if (auth()->user()->hasPermissionTo('Delete Category'))
                                    <td>
